...
<%@ page import="org.sventon.model.ChangeType" %>
...
<c:choose>
    ...
    <c:otherwise>
        <table id="diffTable" cellspacing="0" style="width:100%">
            <tr>
                <th width="100%" style="background-color: white;">${command.fromPath} @
                    revision ${command.fromRevision}</th>
            </tr>
            <tr>
                <th width="100%" style="background-color: white;">${command.toPath} @
                    revision ${command.toRevision}</th>
            </tr>
            <tr>
                <td>
                    <table class="sventonPathDiffTable">
                        ...
                        <c:forEach items="${diffResult}" var="row">
                            ...
                            <tr>
                                ...
                                <% if (ChangeType.ADDED == changeType || ChangeType.REPLACED == changeType) { %>
                                <td><a href="${goToUrl}?revision=${command.toRevision}"
                                       title="Show file">${row.path}</a></td>
                                <% } else if (ChangeType.DELETED == changeType) { %>
                                <td><a href="${goToUrl}?revision=${command.fromRevision}"
                                       title="Show file">${row.path}</a></td>
                                <% } else if (ChangeType.MODIFIED == changeType) { %>
                                <td>
                                    <a href="${diffUrl}&entries=${command.fromPath}/${row.path}@${command.fromRevision}&entries=${command.toPath}/${row.path}@${command.toRevision}"
                                       title="Show Diff">${row.path}</a></td>
                                <% } else { %>
                                <td>${row.path}</td>
                                <% } %>
                                <td><%= BooleanUtils.toStringYesNo(row.isPropertyModified()) %>
                                </td>
                            </tr>
                        </c:forEach>
                        <tr>
