...
public interface DirEntryCache extends Cache {
   ...
   */
  void add(final DirEntry... entries);
   ...
   */
  void update(final Map<String, DirEntry.Kind> entriesToDelete, final List<DirEntry> entriesToAdd);
   ...
   */
  List<DirEntry> findEntries(String searchString, String startDir);

  /**
   * Finds directories recursively.
   *
   * @param startPath Start path
   * @return List of entries (of type {@link org.sventon.model.DirEntry.Kind#DIR}.
   */
  List<DirEntry> findDirectories(final String startPath);
   ...
   */
  List<DirEntry> findEntriesByCamelCasePattern(final CamelCasePattern camelCasePattern, final String startPath);
}
