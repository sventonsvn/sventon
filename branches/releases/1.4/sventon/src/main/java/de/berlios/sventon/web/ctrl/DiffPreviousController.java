...
public final class DiffPreviousController extends AbstractSVNTemplateController implements Controller {
  ...
  protected ModelAndView svnHandle(final SVNRepository repository, final SVNBaseCommand svnCommand,
    ...
    final long revNumber = revision == HEAD ? getRepositoryService().getLatestRevision(repository) : revision.getNumber();
    final List<SVNFileRevision> revisions = getRepositoryService().getFileRevisions(repository, svnCommand.getPath(), revNumber);
    ...
    try {
      final List<SideBySideDiffRow> diffResult = getRepositoryService().diffSideBySide(repository, diffCommand,
          SVNRevision.UNDEFINED, userRepositoryContext.getCharset(), getInstanceConfiguration(svnCommand.getName()));
