...
public class LogEntryCacheImplTest extends TestCase {
  ...
  public void testAddAndFindWithStartDir() throws Exception {
    ...
    cache.add(new LogEntry(create(123, "First message XYZ-123.",
    ...
    cache.add(new LogEntry(create(456, "First message XYZ-456.",
  ...
  public void testAddAndFind() throws Exception {
    ...
    List<LogEntry> logEntries = cache.find("XYZ-123");
    assertEquals(1, logEntries.size());
    assertEquals("First message <span class=\"searchhit\">XYZ-123</span>.", logEntries.get(0).getMessage());
    ...
    cache.add(new LogEntry(create(124, "This is a log message for revision 124.")));
    assertEquals(2, cache.getSize());

    logEntries = cache.find("message");
    assertEquals(2, logEntries.size());
    cache.add(new LogEntry(create(125, "This is a log message for revision 125.")));
    assertEquals(3, cache.getSize());

    cache.add(new LogEntry(create(126, "This is a log message for revision 126......."))); // punctuation will be trimmed
    ...
    cache.add(new LogEntry(create(127, "Testing brackets (must work)")));
    ...
    cache.add(new LogEntry(create(128, "Testing brackets (must work)")));
    ...
    cache.add(new LogEntry(create(129, "Lite svenska tecken, \u00E5 \u00E4 \u00F6!")));
    ...
    cache.add(new LogEntry(create(130, "<This is a <code>log</code> message for &amp; revision 130.......")));
    ...
    cache.add(new LogEntry(create(131, "Lorem Ipsum is simply dummy text of the printing and typesetting industry. " +
  ...
  public void testAddEmptyAndNull() throws Exception {
    cache.add(new LogEntry(create(123, "")));
    assertEquals(1, cache.getSize());

    cache.add(new LogEntry(create(124, null)));
    assertEquals(2, cache.getSize());

    cache.add(new LogEntry(new SVNLogEntry(null, 125, null, new Date(), "abc")));
    assertEquals(3, cache.getSize());
    final LogEntry logEntry = cache.find("abc").get(0);
  ...
  public void testUsingAndLogic() throws Exception {
    cache.add(new LogEntry(create(234, "one two three")));
    assertEquals(1, cache.getSize());
    final LogEntry logEntry = cache.find("one AND two").get(0);
